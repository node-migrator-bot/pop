...
function SiteBuilder(config, fileMap) {
  ...
  if (fs.existsSync(postFilterFile)) {
  ...
  function build() {
    ...
    function renderPost() {
      if (posts.length) {
        ...
        // 'other files' are templates and might depend on posts
...
SiteBuilder.prototype.cacheIncludes = function() {
  ...
  function done() {
    if (toRead === 0) {
      return self.events.emit('cached includes');
    }
    toRead--;
  }

  fs.exists(path.join(this.root, '_includes'), function(exists) {
    if (!exists) {
      done();
    } else {
      fs.readdir(path.join(self.root, '_includes'), function(err, files) {
        ...
        // TODO: Shouldn't this use FileMap?
        files = files.filter(function(name) {
          return !name.match(/^\./);
        });

        if (files.length === 0) done();
        toRead = files.length - 1;
        ...
        for (var i = 0; i < files.length; i++) {
          ...
          if (path.extname(file) !== '.jade') {
            ...
            self.loadCachedInclude(file, done);
          }
        }
      });
    }
...
SiteBuilder.prototype.loadCachedInclude = function(file, done) {
  var self = this;
